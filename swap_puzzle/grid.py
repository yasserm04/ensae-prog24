...
class Grid():
    ...
    def dans_liste(self,G1,G2, liste) :
        for (k,l) in liste :
            if self.egal_matrices(G1,k) and self.egal_matrices(G2,l) :
                return True
        return False
    ...
    def liste_noeuds_voisins(self):
        ...
        for M1 in self.noeuds () :
            ...
            for M2 in self.noeuds ():
                ...
                if not self.egal_matrices(M11, M21):
                    if not self.dans_liste (M11, M21, L) and not self.dans_liste (M21, M11, L):
                ...
                if x not in visited :
                    if x not in g:
                        ...
                        for (i, j) in liste_noeuds_voisins(self):
                            ...
                            if self.egal_matrices(i1, x):
                                g[x].append(i)
                            elif self.egal_matrices(j1, x):
                                g[x].append(j)
                    visited.append(x)
                for voisin in g[x]:
    ...
    def nv_bfs2(grid):
        ...
        while Queue and not(Found) :
            ...
            if Current_grid not in Visited:
                ...
                Next_grids = Grid(len(L),len(L[0]),L).liste_noeuds_voisins()
